...
/* Copyright (c) 2016 URBN INC. All Rights Reserved.
 * @author  - Akshay Dhenge
 * @version - 1.1, 12/19/16
 * @SDK_used - 11.0
 */
 ...
 * @brief Tier_1 Node of MESH
 *
 * This file contains the source code for Tier_1 Node of MESH.
 */
...
#define APP_COMPANY_IDENTIFIER          0x1100                           /**< Company identifier for Nordic Semiconductor ASA. as per www.bluetooth.org. */
...
//----------------------- Function to detect advertisement (called by interupt)-------------------------------//

// Note -  This Function will never be used in this Node (Since first node wont be scanning anything) --------//
...
static void on_adv_report(const ble_evt_t * const p_ble_evt)
{
    ...
    {    
        ...
        adv_data.data_len   = p_gap_evt->params.adv_report.dlen;
				...
				// Check for the device-mesh ID to recieve from 
			  if (adv_data.p_data[5] == 0x03 &&  adv_data.p_data[6] == 0x11){ 
				...
				// checking for ny errors in transmission (parse the data - if correct)
			  if (checksum_correct){
...
int main(void)
{
  ...
  //------------------- temporary functions ----------------------------//
   ...
   advertising_start();
	 ...
	 //SEGGER_RTT_printf(0,"before scanning - %d\n",p_data[9]); // uncomment to debug anything with RTT viewer
    
    for (;; )
    {
			Magmeter_value = nrf_gpio_pin_read(sensor_0); // checking the input from sensor 
			
			//---------------------- transmiting data ----------------------------------//				
			if (ready_trans){
				
					if(Magmeter_value){
						m_beacon_info[0] =   0x80;//
						LEDS_ON(BSP_LED_3_MASK);  // no need here....just for debugging
					}
					else{
						m_beacon_info[0] =   0x00;//
						LEDS_OFF(BSP_LED_3_MASK); // no need here....just for debugging
					}	
					...
					// Splitting the 16-bit into two 8-bit
					temp1 = temp2 = (int16_t)(manuf_specific_data.company_identifier);
					temp1 = (temp1 >> 8);       // Device_ID
					temp2 = (temp2 & 0xFF);     // Mesh_ID
					...
					advdata.p_manuf_specific_data = &manuf_specific_data;
					ble_advdata_set(&advdata, NULL); // Sending the data along with checksum
	   	}		
    }
}
