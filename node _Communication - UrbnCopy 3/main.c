...
#define APP_COMPANY_IDENTIFIER          0x1101                           /**< Company identifier for Nordic Semiconductor ASA. as per www.bluetooth.org. */
...
static void on_adv_report(const ble_evt_t * const p_ble_evt)
{
    ...
    {    
        ...
        adv_data.data_len   = p_gap_evt->params.adv_report.dlen;
			...
			if (adv_data.p_data[5] == 0x00 &&  adv_data.p_data[6] == 0x11){// && adv_data.p_data[29] == sum%256){
				...
				checksum = sum % 127;
				SEGGER_RTT_printf(0,"checksum - 0x%x\n",checksum);
				if (checksum == trial_data[29]){
...
//---------------------- transmiting data ----------------------------------//				
			if (ready_trans){
			 // manuf_specific_data.company_identifier = 0x69;//(p_data[5]+0x10);
				...
				if(sw){
					m_beacon_info[0] =  (0x40 | p_data[7]) ;// 0x80;//
					LEDS_ON(BSP_LED_3_MASK);
				}
				else{
					m_beacon_info[0] =  (0xBF & p_data[7]);// 0x00;//
